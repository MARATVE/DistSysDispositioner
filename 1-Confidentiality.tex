...
\frame{\frametitle{Stream Ciphers}
    ...
    Stream algorithms do not produce truly random outcomes. We cannot output more than $2^{\mathsl{length}}$ different strings. (TJEK DET HER FARMAND SIDE 130) 
}    
...
\frame{\frametitle{OFB: Output Feedback}
    Can be used in a similar way to a stream cipher. Take AES with an encryption function $AES_K()$ as an example block cipher, we can compute
    \begin{align*}
